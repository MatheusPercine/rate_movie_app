...
const RatedMoviesPage = () => {
  ...
  const ratedMovies = useMemo(() => {
    return [...ratings].sort((a, b) => a.title.localeCompare(b.title, "pt-BR"));
  }, [ratings]);
    ...
    <div className="pt-20 pb-12 px-4 container mx-auto min-h-screen">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-destructive">
          <p className="text-lg">Erro ao carregar avaliações.</p>
          <p className="text-sm mt-1 text-muted-foreground">
            {error instanceof Error ? error.message : "Não foi possível buscar filmes avaliados."}
          </p>
        </div>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {paginatedMovies.map((movie) => (
              <MovieCard key={movie.id} movie={movie} rating={movie.userRating} />
            ))}
          </div>
          <Pagination
